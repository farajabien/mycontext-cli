# MyContext Studio (Deprecated)

**Status:** Deprecated / legacy direction.

MyContext CLI is pivoting to a **context generator for AI design tools** (e.g. Google Stitch). The primary workflow is generating `.mycontext/` files and a single `design-prompt.txt` you can paste into an AI designer.

This document is kept for historical reference only. If you’re looking for the current product direction, start here:

- `docs/README.md`
- `docs/01-getting-started/getting-started.md`
- `docs/03-reference/commands.md` (see `generate:design-prompt`)
...
│   └── (studio)/
│       ├── generate/       # Component generation UI
│       ├── library/        # Component library browser
│       └── cli-preview/    # CLI component preview
├── components/
│   ├── generated/          # CLI writes components here
│   ├── studio/            # Studio-specific components
│   └── ui/                # shadcn/ui components
├── lib/
│   ├── component-loader.ts # Load components from CLI
│   └── file-watcher.ts    # Watch for component changes
├── hooks/
│   └── use-cli-components.ts # React hook for CLI components
└── package.json
```
