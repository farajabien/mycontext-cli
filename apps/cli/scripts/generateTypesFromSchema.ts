...
class SchemaTypesGenerator {
  ...
  constructor(
    ...
    if (schemaPath) {
      ...
      if (!found) {
        throw new Error(
          `Schema file not found. Looked in: ${possiblePaths.join(", ")}. ` +
  ...
  async generateTypes(): Promise<void> {
    console.log(`🔄 Generating types from InstantDB schema (${this.schemaPath})...`);
  ...
  private generateTypesContent(schema: InstantDBSchema): string {
    ...
    const withRelationsTypes = this.generateWithRelationsTypes(schema.entities, schema.links || {});
    const insertTypes = this.generateInsertTypes(schema.entities);
 ...
 *
 * This file is automatically generated from ${this.schemaPath}
...
// ============================================================================
// WITH RELATIONS TYPES (for queries with nested data)
// ============================================================================
...
export type ComponentProps<EntityType> = {
  variant?: "mobile" | "desktop";
  className?: string;
} & EntityType;

// Helper to make all fields optional except specified ones
...
${fieldTypes}
  ...
  private generateWithRelationsTypes(
    entities: { [entityName: string]: SchemaEntity },
    links: { [linkName: string]: any }
  ): string {
    if (Object.keys(links).length === 0) {
      return "// No relationships defined in schema";
    }

    // Build a map of entity relationships
    const entityRelations = new Map<string, Array<{ field: string; type: string; isArray: boolean }>>();
    ...
    for (const [linkName, linkDef] of Object.entries(links)) {
      ...
      // Add forward relation (from reverse entity's perspective)
      if (!entityRelations.has(reverseEntity)) {
        entityRelations.set(reverseEntity, []);
      }
      entityRelations.get(reverseEntity)!.push({
        field: forwardLabel,
        type: this.toPascalCase(forwardEntity),
        isArray: linkDef.forward.has === "many",
      });

      // Add reverse relation (from forward entity's perspective)
      if (!entityRelations.has(forwardEntity)) {
        entityRelations.set(forwardEntity, []);
      }
      entityRelations.get(forwardEntity)!.push({
        field: reverseLabel,
        type: this.toPascalCase(reverseEntity),
        isArray: linkDef.reverse.has === "many",
      });
    }

    // Generate WithRelations types
    const withRelationsTypes = Object.keys(entities).map((entityName) => {
      const pascalName = this.toPascalCase(entityName);
      const relations = entityRelations.get(entityName) || [];

      if (relations.length === 0) {
        return `export type ${pascalName}WithRelations = ${pascalName}; // No relations`;
      }

      const relationFields = relations
        .map((rel) => {
          const type = rel.isArray ? `${rel.type}[]` : rel.type;
          return `  ${rel.field}?: ${type};`;
        })
        .join("\n");

      return `export type ${pascalName}WithRelations = ${pascalName} & {
${relationFields}
};`;
    });

    return withRelationsTypes.join("\n\n");
  }

  private generateInsertTypes(entities: {
    [entityName: string]: SchemaEntity;
  }): string {
    const insertTypes = Object.entries(entities).map(([entityName, fields]) => {
      const pascalName = this.toPascalCase(entityName);

      // Common auto-generated fields to omit
      const autoFields = ["id", "createdAt", "created_at", "updatedAt", "updated_at"];

      // Find which fields are auto-generated
      const omittedFields = Object.keys(fields).filter((fieldName) =>
        autoFields.includes(fieldName)
      );

      if (omittedFields.length === 0) {
        // No auto-generated fields, Insert type is same as entity type
        return `export type ${pascalName}Insert = ${pascalName};`;
      }

      const omitClause = omittedFields.map((f) => `"${f}"`).join(" | ");

      return `export type ${pascalName}Insert = Omit<${pascalName}, ${omitClause}>;`;
    });

    return insertTypes.join("\n\n");
  }
