...
interface InstantDBSetupOptions extends CommandOptions {
  mcp?: boolean;
  auth?: boolean;
  schema?: boolean;
  components?: boolean;
...
export class InstantDBSetupCommand {
  private fs = new FileSystemManager();
  private spinner = new EnhancedSpinner("Setting up InstantDB...");

  async execute(options: InstantDBSetupOptions): Promise<void> {
    const {
      mcp = true,
      auth = true,
      schema = true,
    ...
    console.log(chalk.blue.bold("🚀 InstantDB + MCP Setup\n"));
  ...
  private async setupInstantDB(options: {
    ...
    console.log(
      chalk.blue("🚀 Setting up InstantDB with MCP integration...\n")
    );
  ...
  private async installInstantDBDependencies(
    includeMCP: boolean
  ): Promise<void> {
  ...
  private async setupMCPIntegration(token?: string): Promise<void> {
...
`;
    ...
    await fs.writeFile(
      path.join(process.cwd(), "lib", "mcp.ts"),
      mcpConfigContent
    );
...
`;
    ...
    await fs.writeFile(
      path.join(process.cwd(), "lib", "mcp-hooks.ts"),
      mcpHooksContent
    );

    console.log(chalk.gray("   MCP integration files created"));
  }
...
`;
  ...
  private async createEnvironmentConfig(appId?: string): Promise<void> {
...
`;
  ...
  private showNextSteps(): void {
    ...
    console.log(chalk.yellow("2. Get your Personal Access Token (for MCP):"));
    console.log(chalk.gray("   • Visit https://instantdb.com/dash/settings"));
    console.log(chalk.gray("   • Generate a Personal Access Token"));
    console.log(chalk.gray("   • Add it to your .env.local\n"));

    console.log(chalk.yellow("3. Update your environment variables:"));
    console.log(chalk.gray("   • Edit .env.local with your APP_ID and token"));
    console.log(
      chalk.gray("   • Example: NEXT_PUBLIC_INSTANT_APP_ID=your_app_id_here\n")
    );

    console.log(chalk.yellow("4. Push your schema to InstantDB:"));
    console.log(chalk.gray("   • Run: npx instant-cli@latest push"));
    console.log(chalk.gray("   • This will sync your schema with InstantDB\n"));

    console.log(chalk.yellow("5. Test your setup:"));
    ...
    console.log(
      chalk.yellow("6. Generate components with database integration:")
    );
    ...
    console.log(
      chalk.gray(
        "   • MCP Integration: https://github.com/instantdb/instant/tree/main/client/packages/mcp"
      )
    );
    console.log(
      chalk.gray(
