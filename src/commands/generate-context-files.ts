...
export class GenerateContextFilesCommand {
  ...
  async execute(options: ContextFilesOptions): Promise<void> {
    ...
    if (validationResult.hasCriticalGaps) {
      ...
      const criticalGaps = validationResult.gaps.filter(
      ...
      criticalGaps.forEach((gap, index) => {
        ...
        if (gap.suggestedAnswers) {
          gap.suggestedAnswers.forEach((answer, i) => {
            console.log(
              chalk.gray(`   - ${String.fromCharCode(97 + i)}) ${answer}`)
            );
          });
        }
      });
      ...
      console.log(
        chalk.blue(
          "   Or run 'mycontext review:context' to address these gaps interactively."
        )
      );
      return;
    }
    ...
    if (validationResult.autoGeneratedFeatures.length > 0) {
      ...
      validationResult.autoGeneratedFeatures.forEach((feature, index) => {
        console.log(chalk.gray(`   ${index + 1}. ${feature.feature}`));
   ...
   * Save auto-generated features for review
   */
  private async saveAutoGeneratedFeatures(
    contextDir: string,
    ...
    await fs.writeFile(
      path.join(contextDir, "auto-generated.json"),
      JSON.stringify(autoGeneratedData, null, 2)
    );
  }
