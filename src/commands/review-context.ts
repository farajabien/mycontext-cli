...
export class ReviewContextCommand {
  ...
  async execute(options: ReviewContextOptions): Promise<void> {
    console.log(
      chalk.blue("🔍 Reviewing context files and auto-generated features...")
    );
    ...
    try {
      ...
      if (!(await this.fs.exists(autoGeneratedPath))) {
        console.log(
          chalk.yellow(
            "⚠️  No auto-generated features found. Run 'mycontext generate:context' first."
          )
        );
        return;
      }
      ...
      if (features.length === 0 && gaps.length === 0) {
        console.log(chalk.green("✅ No features or gaps to review."));
        return;
      }
      ...
      console.log(chalk.green("✅ Context review completed!"));
    } catch (error) {
      console.error(chalk.red("❌ Context review failed:"), error);
      throw error;
    }
  }
  ...
  private async editFeature(feature: AutoGeneratedFeature): Promise<void> {
    ...
    if (response.editedFeature) {
      ...
      console.log(chalk.green("   ✏️  Feature updated"));
    }
  }
