...
class TrainingDataGenerator {
  ...
  private generateVariation(mapping: IntentMapping, canonicalName: string, index: number): { userPrompt: string; assistantResponse: string } {
    ...
    const assistantResponse = this.generateComponentCode(mapping, basePhrase, canonicalName)
    
    return { userPrompt, assistantResponse }
  }

  private generateComponentCode(mapping: IntentMapping, phrase: any, canonicalName: string): string {
    ...
    const explanation = this.generateIntentExplanation(mapping, phrase, canonicalName)
  ...
  private expandTemplate(template: string, mapping: IntentMapping): string {
    ...
    const imports = mapping.component_pattern.shadcn_components
      .map(comp => `import { ${comp.name} } from '@/components/ui/${comp.name.toLowerCase()}'`)
      .join('\n')
    ...
    const componentName = this.generateComponentName(mapping.component_pattern.pattern_name)
  ...
  private generateIntentExplanation(mapping: IntentMapping, phrase: any, canonicalName: string): string {
    return [
      `- Primary Intent: \`${canonicalName}\` (confidence: 0.95)`,
      `- Components Used: ${mapping.component_pattern.shadcn_components.map(c => c.name).join(', ')}`,
      `- Design Pattern: ${mapping.component_pattern.pattern_name}`,
      `- Required Props: ${mapping.component_pattern.required_props.map(p => p.name).join(', ')}`,
...
- Available Components: ${mapping.component_pattern.shadcn_components.map(c => c.name).join(', ')}
- Design Pattern: ${mapping.component_pattern.pattern_name}
- Required Props: ${mapping.component_pattern.required_props.map(p => `${p.name}: ${p.type}`).join(', ')}
- Accessibility Specs: ${JSON.stringify(mapping.accessibility_spec)}`
  ...
  private generateContextualVariations(): TrainingExample[] {
    ...
    for (const [canonicalName, mapping] of Object.entries(this.dictionary.mappings)) {
      for (const domain of this.domains) {
        ...
        for (let i = 0; i < 2; i++) {
          const domainExample = this.generateDomainExample(mapping, canonicalName, domain, i)
  ...
  private generateDomainExample(mapping: IntentMapping, canonicalName: string, domain: string, index: number): TrainingExample {
    const phrases = mapping.intent_phrases
    const basePhrase = phrases[Math.floor(Math.random() * phrases.length)]
    ...
    const compositions = [
      {
        ...
        system: "Generate a complete product page combining multiple UI components with proper layout and interactions."
      },
      {
        ...
        system: "Create a comprehensive dashboard layout with multiple data visualization components."
      },
      {
        ...
        system: "Build a user profile interface with multiple interactive components and data sections."
      },
      {
        user: "Create a blog post editor with rich text, image upload, tags, and preview panel",
        components: ['rich-text-editor', 'file-upload', 'tag-input', 'preview-panel'],
        system: "Generate a content creation interface with editing and preview capabilities."
      },
      {
        user: "Build a checkout flow with cart summary, shipping form, payment method, and order confirmation",
        components: ['cart-summary', 'form', 'payment-form', 'confirmation-dialog'],
        system: "Create a multi-step checkout process with form validation and payment integration."
      }
    ]
  ...
  private generateCompositionCode(components: string[]): string {
    ...
    const imports = components.map(comp => {
      ...
      return mapping?.component_pattern.shadcn_components.map(c => c.name).join(', ') || comp
    }).join(', ')
    ...
    const edgeCases = [
      {
        ...
        system: "Generate components with comprehensive error handling, loading states, and accessibility features."
      },
      {
        user: "Add a data table with empty state, loading skeleton, and error handling",
        scenario: "empty-loading",
        system: "Create data components with proper state management and user feedback."
      },
      {
        user: "Build a component with disabled state, tooltip explanation, and keyboard navigation",
        scenario: "disabled-tooltip",
        system: "Generate accessible components with proper interaction states and keyboard support."
      },
      {
        user: "Create a responsive component that works on mobile, tablet, and desktop",
        scenario: "responsive",
        system: "Build components with responsive design patterns and mobile-first approach."
      },
      {
        user: "Add a component with dark mode support and theme switching",
        scenario: "theming",
        system: "Generate components with theme support and proper color contrast."
      }
    ]
...
async function main() {
  ...
  const dictionaryPath = args[0] || path.join(__dirname, '..', 'config', 'intent-dictionary.json')
  const outputPath = args[1] || path.join(__dirname, '..', 'training-data', 'mycontext-ai-training.jsonl')
